...
fun ShowMediaInfo(videoId: String, onClose: () -> Unit) {
    ...
    LazyColumn(Modifier.fillMaxWidth(), rememberLazyListState(), PaddingValues(16.dp, 0.dp, 16.dp, 24.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
        ...
        item { Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(12.dp)) { FilledTonalButton({ copyToClip(videoId) }, Modifier.weight(1f)) { Icon(painterResource(R.drawable.copy), null); Spacer(Modifier.width(8.dp)); Text("Copy") }; OutlinedButton({ ctx.startActivity(Intent.createChooser(Intent(Intent.ACTION_SEND).apply { type = "text/plain"; putExtra(Intent.EXTRA_TEXT, "https://music.youtube.com/watch?v=$videoId") }, null)) }, Modifier.weight(1f)) { Icon(painterResource(R.drawable.share), null); Spacer(Modifier.width(8.dp)); Text("Share") } } }
        ...
        item {
            AnimatedContent(tab, transitionSpec = { fadeIn() togetherWith fadeOut() }, label = "") { t ->
                Column(Modifier.fillMaxWidth().animateContentSize(), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                    ...
                    when (t) {
                        Tab.Info -> { InfoListCard(listOf("Song Title" to (song?.title ?: unk), "Artists" to (song?.artists?.takeIf { it.isNotEmpty() }?.joinToString { it.name } ?: unk), "Media ID" to videoId), copyToClip); ElevatedCard(Modifier.fillMaxWidth(), colors = CardDefaults.elevatedCardColors(MaterialTheme.colorScheme.surfaceContainerLow)) { Column(Modifier.fillMaxWidth().padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) { Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) { Text("Description", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.SemiBold); OutlinedButton({ copyToClip(unk) }) { Icon(painterResource(R.drawable.copy), null); Spacer(Modifier.width(8.dp)); Text("Copy") } }; Text(unk, style = MaterialTheme.typography.bodyLarge, color = MaterialTheme.colorScheme.onSurfaceVariant) } } }
                        ...
                        Tab.Stats -> { Column(Modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(12.dp)) { listOf("Subscribers" to unk, "Views" to unk, "Likes" to unk, "Dislikes" to unk).chunked(2).forEach { row -> Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(12.dp)) { row.forEach { (l, v) -> ElevatedCard(Modifier.weight(1f), colors = CardDefaults.elevatedCardColors(MaterialTheme.colorScheme.surfaceContainerLow)) { Column(Modifier.fillMaxWidth().padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) { Text(l, style = MaterialTheme.typography.labelLarge, color = MaterialTheme.colorScheme.onSurfaceVariant); Text(v, style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.SemiBold) } } }; if (row.size == 1) Spacer(Modifier.weight(1f)) } } } }
                    }
                }
            }
        }
