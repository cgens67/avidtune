...
fun LazyItemScope.fluidAnim() = Modifier.animateItem(
...
@Composable fun LibraryScreen(navController: NavController) {
    ...
    Box(Modifier.fillMaxSize()) {
        VerticalFastScroller(listState = lazyListState, modifier = Modifier, topContentPadding = 16.dp, endContentPadding = 0.dp) {
            AnimatedContent(
                targetState = filterType,
                transitionSpec = {
                    val enter = fadeIn(tween(400, easing = FastOutSlowInEasing)) + slideInHorizontally(tween(500, easing = FastOutSlowInEasing), initialOffsetX = { 50 })
                    val exit = fadeOut(tween(400, easing = FastOutSlowInEasing)) + slideOutHorizontally(tween(500, easing = FastOutSlowInEasing), targetOffsetX = { -50 })
...
@Composable fun LibraryMixScreen(navController: NavController, filterContent: @Composable () -> Unit, viewModel: LibraryMixViewModel = hiltViewModel()) {
    ...
    val allItemsUnsorted = viewModel.albums.collectAsState().value + viewModel.artists.collectAsState().value + viewModel.playlists.collectAsState().value
    val allItems = remember(allItemsUnsorted, sortType, sortDescending) {
        val collator = Collator.getInstance(Locale.getDefault()).apply { strength = Collator.PRIMARY }
        val sorted = when (sortType) { MixSortType.CREATE_DATE -> allItemsUnsorted.sortedBy { when (it) { is Album -> it.album.bookmarkedAt; is Artist -> it.artist.bookmarkedAt; is Playlist -> it.playlist.createdAt; else -> LocalDateTime.now() } }; MixSortType.NAME -> allItemsUnsorted.sortedWith(compareBy(collator) { when (it) { is Album -> it.album.title; is Artist -> it.artist.name; is Playlist -> it.playlist.name; else -> "" } }); MixSortType.LAST_UPDATED -> allItemsUnsorted.sortedBy { when (it) { is Album -> it.album.lastUpdateTime; is Artist -> it.artist.lastUpdateTime; is Playlist -> it.playlist.lastUpdateTime; else -> LocalDateTime.now() } } }
        ...
        AnimatedContent(targetState = viewType, transitionSpec = { val enter = fadeIn(tween(400, easing = FastOutSlowInEasing)); val exit = fadeOut(tween(400, easing = FastOutSlowInEasing)); enter togetherWith exit }, label = "") { vt ->
            if (vt == LibraryViewType.LIST) {
                LazyColumn(state = lazyListState, contentPadding = LocalPlayerAwareWindowInsets.current.asPaddingValues()) {
                    ...
                    item(key = "likedPlaylist", contentType = { CONTENT_TYPE_PLAYLIST }) { PlaylistListItem(playlist = likedPlaylist, autoPlaylist = true, modifier = Modifier.fillMaxWidth().clickable { navController.navigate("auto_playlist/liked") }.then(fluidAnim())) }
                    item(key = "downloadedPlaylist", contentType = { CONTENT_TYPE_PLAYLIST }) { PlaylistListItem(playlist = downloadPlaylist, autoPlaylist = true, modifier = Modifier.fillMaxWidth().clickable { navController.navigate("auto_playlist/downloaded") }.then(fluidAnim())) }
                    item(key = "TopPlaylist", contentType = { CONTENT_TYPE_PLAYLIST }) { PlaylistListItem(playlist = topPlaylist, autoPlaylist = true, modifier = Modifier.fillMaxWidth().clickable { navController.navigate("top_playlist/$topSize") }.then(fluidAnim())) }
                    item(key = "cachePlaylist", contentType = { CONTENT_TYPE_PLAYLIST }) { PlaylistListItem(playlist = cachePlaylist, autoPlaylist = true, modifier = Modifier.fillMaxWidth().clickable { navController.navigate("cache_playlist/cached") }.then(fluidAnim())) }
                    items(items = allItems, key = { it.id }, contentType = { CONTENT_TYPE_PLAYLIST }) { item ->
                        when (item) {
                            is Playlist -> PlaylistListItem(playlist = item, trailingContent = { IconButton(onClick = { menuState.show { PlaylistMenu(playlist = item, coroutineScope = coroutineScope, onDismiss = { menuState.dismiss() }) } }) { Icon(painterResource(R.drawable.more_vert), null) } }, modifier = Modifier.fillMaxWidth().combinedClickable(onClick = { navController.navigate("local_playlist/${item.id}") }, onLongClick = { haptic.performHapticFeedback(HapticFeedbackType.LongPress); menuState.show { PlaylistMenu(playlist = item, coroutineScope = coroutineScope, onDismiss = { menuState.dismiss() }) } }).then(fluidAnim()))
                            is Artist -> ArtistListItem(artist = item, trailingContent = { IconButton(onClick = { menuState.show { ArtistMenu(originalArtist = item, coroutineScope = coroutineScope, onDismiss = { menuState.dismiss() }) } }) { Icon(painterResource(R.drawable.more_vert), null) } }, modifier = Modifier.fillMaxWidth().combinedClickable(onClick = { navController.navigate("artist/${item.id}") }, onLongClick = { haptic.performHapticFeedback(HapticFeedbackType.LongPress); menuState.show { ArtistMenu(originalArtist = item, coroutineScope = coroutineScope, onDismiss = { menuState.dismiss() }) } }).then(fluidAnim()))
                            is Album -> AlbumListItem(album = item, isActive = item.id == mediaMetadata?.album?.id, isPlaying = isPlaying, trailingContent = { IconButton(onClick = { menuState.show { AlbumMenu(originalAlbum = item, navController = navController, onDismiss = { menuState.dismiss() }) } }) { Icon(painterResource(R.drawable.more_vert), null) } }, modifier = Modifier.fillMaxWidth().combinedClickable(onClick = { navController.navigate("album/${item.id}") }, onLongClick = { haptic.performHapticFeedback(HapticFeedbackType.LongPress); menuState.show { AlbumMenu(originalAlbum = item, navController = navController, onDismiss = { menuState.dismiss() }) } }).then(fluidAnim()))
                            else -> {}
                        }
                    }
                }
                ...
                LazyVerticalGrid(state = lazyGridState, columns = GridCells.Adaptive(GridThumbnailHeight + if (gridItemSize == GridItemSize.BIG) 24.dp else (-24).dp), contentPadding = LocalPlayerAwareWindowInsets.current.asPaddingValues()) {
                    ...
                    item(key = "likedPlaylist", contentType = { CONTENT_TYPE_PLAYLIST }) { PlaylistGridItem(playlist = likedPlaylist, fillMaxWidth = true, autoPlaylist = true, modifier = Modifier.fillMaxWidth().combinedClickable(onClick = { navController.navigate("auto_playlist/liked") }).then(fluidAnim()), context = LocalContext.current) }
                    item(key = "downloadedPlaylist", contentType = { CONTENT_TYPE_PLAYLIST }) { PlaylistGridItem(playlist = downloadPlaylist, fillMaxWidth = true, autoPlaylist = true, modifier = Modifier.fillMaxWidth().combinedClickable(onClick = { navController.navigate("auto_playlist/downloaded") }).then(fluidAnim()), context = LocalContext.current) }
                    item(key = "TopPlaylist", contentType = { CONTENT_TYPE_PLAYLIST }) { PlaylistGridItem(playlist = topPlaylist, fillMaxWidth = true, autoPlaylist = true, modifier = Modifier.fillMaxWidth().combinedClickable(onClick = { navController.navigate("top_playlist/$topSize") }).then(fluidAnim()), context = LocalContext.current) }
                    item(key = "cachePlaylist", contentType = { CONTENT_TYPE_PLAYLIST }) { PlaylistGridItem(playlist = cachePlaylist, fillMaxWidth = true, autoPlaylist = true, modifier = Modifier.fillMaxWidth().combinedClickable(onClick = { navController.navigate("cache_playlist/cached") }).then(fluidAnim()), context = LocalContext.current) }
                    items(items = allItems, key = { it.id }, contentType = { CONTENT_TYPE_PLAYLIST }) { item ->
                        when (item) {
                            is Playlist -> PlaylistGridItem(playlist = item, fillMaxWidth = true, modifier = Modifier.fillMaxWidth().combinedClickable(onClick = { navController.navigate("local_playlist/${item.id}") }, onLongClick = { haptic.performHapticFeedback(HapticFeedbackType.LongPress); menuState.show { PlaylistMenu(playlist = item, coroutineScope = coroutineScope, onDismiss = { menuState.dismiss() }) } }).then(fluidAnim()), context = LocalContext.current)
                            is Artist -> ArtistGridItem(artist = item, fillMaxWidth = true, modifier = Modifier.fillMaxWidth().combinedClickable(onClick = { navController.navigate("artist/${item.id}") }, onLongClick = { haptic.performHapticFeedback(HapticFeedbackType.LongPress); menuState.show { ArtistMenu(originalArtist = item, coroutineScope = coroutineScope, onDismiss = { menuState.dismiss() }) } }).then(fluidAnim()))
        ...
        AnimatedContent(targetState = viewType, transitionSpec = { val enter = fadeIn(tween(400, easing = FastOutSlowInEasing)); val exit = fadeOut(tween(400, easing = FastOutSlowInEasing)); enter togetherWith exit }, label = "") { vt ->
            if (vt == LibraryViewType.LIST) {
                LazyColumn(state = lazyListState, contentPadding = LocalPlayerAwareWindowInsets.current.asPaddingValues()) {
                    ...
                    items(items = playlists, key = { it.id }, contentType = { CONTENT_TYPE_PLAYLIST }) { LibraryPlaylistListItem(navController = navController, menuState = menuState, coroutineScope = coroutineScope, playlist = it, modifier = Modifier.then(fluidAnim())) }
                }
                HideOnScrollFAB(visible = true, lazyListState = lazyListState, icon = R.drawable.add, onClick = { showCreatePlaylistDialog = true })
            } else {
                LazyVerticalGrid(state = lazyGridState, columns = GridCells.Adaptive(GridThumbnailHeight + if (gridItemSize == GridItemSize.BIG) 24.dp else (-24).dp), contentPadding = LocalPlayerAwareWindowInsets.current.asPaddingValues()) {
                    item(key = "filter", span = { GridItemSpan(maxLineSpan) }, contentType = CONTENT_TYPE_HEADER) { filterContent() }; item(key = "header", span = { GridItemSpan(maxLineSpan) }, contentType = CONTENT_TYPE_HEADER) { headerContent() }
                    item(key = "likedPlaylist", contentType = { CONTENT_TYPE_PLAYLIST }) { PlaylistGridItem(playlist = likedPlaylist, fillMaxWidth = true, autoPlaylist = true, modifier = Modifier.fillMaxWidth().combinedClickable(onClick = { navController.navigate("auto_playlist/liked") }).then(fluidAnim()), context = LocalContext.current) }
                    ...
                    items(items = playlists, key = { it.id }, contentType = { CONTENT_TYPE_PLAYLIST }) { LibraryPlaylistGridItem(navController = navController, menuState = menuState, coroutineScope = coroutineScope, playlist = it, modifier = Modifier.then(fluidAnim()), context = LocalContext.current) }
                }
                HideOnScrollFAB(visible = true, lazyListState = lazyGridState, icon = R.drawable.add, onClick = { showCreatePlaylistDialog = true })
            }
        }
    }
}
...
@Composable fun LibrarySongsScreen(navController: NavController, onDeselect: () -> Unit, viewModel: LibrarySongsViewModel = hiltViewModel()) {
    ...
    val wrappedSongs = remember(songs) { songs.map { ItemWrapper(it) }.toMutableList() }; var selection by remember { mutableStateOf(false) }
        ...
        VerticalFastScroller(listState = lazyListState, modifier = Modifier, topContentPadding = 16.dp, endContentPadding = 0.dp) {
            LazyColumn(state = lazyListState, contentPadding = LocalPlayerAwareWindowInsets.current.asPaddingValues()) {
                ...
                item(key = "header", contentType = CONTENT_TYPE_HEADER) {
                    AnimatedContent(targetState = selection, transitionSpec = { val enter = fadeIn(tween(300, easing = FastOutSlowInEasing)) + slideInVertically(tween(400, easing = FastOutSlowInEasing), initialOffsetY = { -it }); val exit = fadeOut(tween(300, easing = FastOutSlowInEasing)) + slideOutVertically(tween(400, easing = FastOutSlowInEasing), targetOffsetY = { it }); enter togetherWith exit }, label = "") { sel ->
    ...
    Box(Modifier.fillMaxSize()) {
        AnimatedContent(targetState = viewType, transitionSpec = { val enter = fadeIn(tween(400, easing = FastOutSlowInEasing)); val exit = fadeOut(tween(400, easing = FastOutSlowInEasing)); enter togetherWith exit }, label = "") { vt ->
            if (vt == LibraryViewType.LIST) {
                LazyColumn(state = lazyListState, contentPadding = LocalPlayerAwareWindowInsets.current.asPaddingValues()) {
                    item(key = "filter", contentType = CONTENT_TYPE_HEADER) { filterContent() }; item(key = "header", contentType = CONTENT_TYPE_HEADER) { headerContent() }
                    if (albums.isEmpty()) item { var show by remember { mutableStateOf(false) }; LaunchedEffect(Unit) { show = true }; AnimatedVisibility(visible = show, enter = fadeIn(tween(400, easing = FastOutSlowInEasing)) + expandVertically(tween(500, easing = FastOutSlowInEasing))) { EmptyPlaceholder(icon = R.drawable.album, text = stringResource(R.string.library_album_empty), modifier = Modifier.then(fluidAnim())) } }
                    items(items = albums, key = { it.id }, contentType = { CONTENT_TYPE_ALBUM }) { LibraryAlbumListItem(navController = navController, menuState = menuState, album = it, isActive = it.id == mediaMetadata?.album?.id, isPlaying = isPlaying, modifier = Modifier.then(fluidAnim())) }
                }
            } else {
                LazyVerticalGrid(state = lazyGridState, columns = GridCells.Adaptive(GridThumbnailHeight + if (gridItemSize == GridItemSize.BIG) 24.dp else (-24).dp), contentPadding = LocalPlayerAwareWindowInsets.current.asPaddingValues()) {
                    item(key = "filter", span = { GridItemSpan(maxLineSpan) }, contentType = CONTENT_TYPE_HEADER) { filterContent() }; item(key = "header", span = { GridItemSpan(maxLineSpan) }, contentType = CONTENT_TYPE_HEADER) { headerContent() }
                    ...
                    items(items = albums, key = { it.id }, contentType = { CONTENT_TYPE_ALBUM }) { LibraryAlbumGridItem(navController = navController, menuState = menuState, coroutineScope = coroutineScope, album = it, isActive = it.id == mediaMetadata?.album?.id, isPlaying = isPlaying, modifier = Modifier.then(fluidAnim())) }
                }
            }
        }
    }
    ...
    Box(Modifier.fillMaxSize()) {
        AnimatedContent(targetState = viewType, transitionSpec = { val enter = fadeIn(tween(400, easing = FastOutSlowInEasing)); val exit = fadeOut(tween(400, easing = FastOutSlowInEasing)); enter togetherWith exit }, label = "") { vt ->
            if (vt == LibraryViewType.LIST) {
                LazyColumn(state = lazyListState, contentPadding = LocalPlayerAwareWindowInsets.current.asPaddingValues()) {
                    item(key = "filter", contentType = CONTENT_TYPE_HEADER) { filterContent() }; item(key = "header", contentType = CONTENT_TYPE_HEADER) { headerContent() }
                    if (artists.isEmpty()) item { var show by remember { mutableStateOf(false) }; LaunchedEffect(Unit) { show = true }; AnimatedVisibility(visible = show, enter = fadeIn(tween(400, easing = FastOutSlowInEasing)) + expandVertically(tween(500, easing = FastOutSlowInEasing))) { EmptyPlaceholder(icon = R.drawable.artist, text = stringResource(R.string.library_artist_empty), modifier = Modifier.then(fluidAnim())) } }
                    items(items = artists, key = { it.id }, contentType = { CONTENT_TYPE_ARTIST }) { LibraryArtistListItem(navController = navController, menuState = menuState, coroutineScope = coroutineScope, modifier = Modifier.then(fluidAnim()), artist = it) }
                }
            } else {
                LazyVerticalGrid(state = lazyGridState, columns = GridCells.Adaptive(GridThumbnailHeight + if (gridItemSize == GridItemSize.BIG) 24.dp else (-24).dp), contentPadding = LocalPlayerAwareWindowInsets.current.asPaddingValues()) {
                    item(key = "filter", span = { GridItemSpan(maxLineSpan) }, contentType = CONTENT_TYPE_HEADER) { filterContent() }; item(key = "header", span = { GridItemSpan(maxLineSpan) }, contentType = CONTENT_TYPE_HEADER) { headerContent() }
                    ...
                    items(items = artists, key = { it.id }, contentType = { CONTENT_TYPE_ARTIST }) { LibraryArtistGridItem(navController = navController, menuState = menuState, coroutineScope = coroutineScope, modifier = Modifier.then(fluidAnim()), artist = it) }
                }
            }
        }
    }
...
@Composable fun CachePlaylistScreen(navController: NavController, scrollBehavior: TopAppBarScrollBehavior, viewModel: HistoryViewModel = hiltViewModel()) {
    ...
    Box(Modifier.fillMaxSize()) {
        LazyColumn(state = lazyListState, contentPadding = LocalPlayerAwareWindowInsets.current.asPaddingValues()) {
            if (filteredSongs.isEmpty()) item { var show by remember { mutableStateOf(false) }; LaunchedEffect(Unit) { show = true }; AnimatedVisibility(visible = show, enter = fadeIn(tween(400, easing = FastOutSlowInEasing)) + expandVertically(tween(500, easing = FastOutSlowInEasing))) { EmptyPlaceholder(icon = R.drawable.music_note, text = stringResource(R.string.playlist_is_empty), modifier = Modifier) } }
            else {
                if (!isSearching) item {
                ...
                itemsIndexed(items = filteredSongs, key = { _, s -> s.item.id }) { index, s ->
                    SongListItem(song = s.item, isActive = s.item.id == mediaMetadata?.id, isPlaying = isPlaying, isSelected = s.isSelected && selection, showInLibraryIcon = true, trailingContent = { IconButton(onClick = { menuState.show { SongMenu(originalSong = s.item, navController = navController, onDismiss = { menuState.dismiss() }) } }) { Icon(painterResource(R.drawable.more_vert), null) } }, modifier = Modifier.fillMaxWidth().combinedClickable(onClick = { if (!selection) { if (s.item.id == mediaMetadata?.id) playerConnection.player.togglePlayPause() else playerConnection.playQueue(ListQueue("Cache Songs", filteredSongs.map { it.item.toMediaItem() }, index)) } else s.isSelected = !s.isSelected }, onLongClick = { haptic.performHapticFeedback(HapticFeedbackType.LongPress); if (!selection) selection = true; wrappedSongs.forEach { it.isSelected = false }; s.isSelected = true }).then(fluidAnim()))
                }
            }
        }
        TopAppBar(title = {
            AnimatedContent(targetState = Triple(selection, isSearching, wrappedSongs.count { it.isSelected }), transitionSpec = { val enter = fadeIn(tween(300, easing = FastOutSlowInEasing)) + slideInVertically(tween(400, easing = FastOutSlowInEasing), initialOffsetY = { 50 }); val exit = fadeOut(tween(300, easing = FastOutSlowInEasing)) + slideOutVertically(tween(400, easing = FastOutSlowInEasing), targetOffsetY = { -50 }); enter togetherWith exit }, label = "") { (sel, search, count) ->
                when {
        ...
        }, navigationIcon = { AnimatedContent(targetState = selection, transitionSpec = { val enter = scaleIn(tween(300, easing = FastOutSlowInEasing)); val exit = scaleOut(tween(300, easing = FastOutSlowInEasing)); enter togetherWith exit }, label = "") { sel -> IconButton(onClick = { when { isSearching -> { isSearching = false; query = TextFieldValue(); focusManager.clearFocus() }; selection -> selection = false; else -> navController.navigateUp() } }) { Icon(painterResource(if (sel) R.drawable.close else R.drawable.arrow_back), null) } } }, actions = {
            AnimatedContent(targetState = Pair(selection, isSearching), transitionSpec = { val enter = fadeIn(tween(300, easing = FastOutSlowInEasing)); val exit = fadeOut(tween(300, easing = FastOutSlowInEasing)); enter togetherWith exit }, label = "") { (sel, search) ->
                Row {
